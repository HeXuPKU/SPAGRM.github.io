...
- `N-3` and `N-4`'s IBD-sharing probabilities are $\delta$<sub>ij</sub><sup>(0)</sup> = 0.25, $\delta$<sub>ij</sub><sup>(1)</sup> = 0.5, and $\delta$<sub>ij</sub><sup>(2)</sup> = 0.25. The kinship coefficient is 1/2 $\times$ 0.25 + 1/4 $\times$ 0.5 = 0.25.
  - If genotypes of `N-1` and `N-2` are both <code style="color : orange">Aa</code> in one locus, then there is a 0.25 probability that the genotype of `N-3` is <code style="color : orange">AA</code>, a 0.5 probability that `N-3` is <code style="color : orange">Aa</code>, and a 0.25 probability that `N-3` is <code style="color : orange">aa</code>, so as `N-4`. Therefore, `N-3` and `N-4` have a 0.25 probability of sharing two alleles, a 0.5 probability of sharing one allele, and a 0.25 probability of sharing no alleles in one locus.
